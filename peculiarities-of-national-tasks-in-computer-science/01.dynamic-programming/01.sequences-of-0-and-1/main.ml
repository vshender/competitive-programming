(** 1.1. Sequences of 0 and 1 *)

(** Arbitrary-precision arithmetic on natural numbers. *)
module BigNat = struct
  (** A big natural number stored as an array of digits in base {!base},
      least significant digit first. *)
  type t = {
    num: int array;
    len: int;
  }

  (** The base of the number system. *)
  let base = 10000

  (** The number of decimal digits per base digit. *)
  let dlen = 4

  (** The number 1. *)
  let one = { num = [| 1 |]; len = 1 }

  (** The number 2. *)
  let two = { num = [| 2 |]; len = 1 }

  (** [normalize x] trims leading zero digits from [x]. *)
  let normalize x =
    let rec find_len i =
      if i = 1 || x.num.(i - 1) > 0 then
        i
      else
        find_len (i - 1)
    in { x with len = find_len x.len }

  (** [add x y] returns the sum of [x] and [y]. *)
  let add x y =
    let maxlen = Int.max x.len y.len in
    let num = Array.make (maxlen + 1) 0 in
    let carry = ref 0 in
    for i = 0 to maxlen do
      let xi = if i < x.len then x.num.(i) else 0
      and yi = if i < y.len then y.num.(i) else 0 in
      let sum = xi + yi + !carry in
      num.(i) <- sum mod base;
      carry := sum / base
    done;
    normalize { num; len = maxlen + 1 }

  (** [to_string x] returns the decimal string representation of [x]. *)
  let to_string { num; len } =
    let buf = Buffer.create (len * dlen) in

    num.(len - 1) |> string_of_int |> Buffer.add_string buf;
    for i = len - 2 downto 0 do
      let s = string_of_int num.(i) in
      for _ = 0 to dlen - (String.length s) - 1 do
        Buffer.add_char buf '0'
      done;
      Buffer.add_string buf s
    done;

    Buffer.contents buf
end

(** [solve n] returns the number of binary sequences of length [n] with no
    two adjacent 1s.

    A valid sequence either ends with 0 (any valid prefix of length n-1) or
    ends with 01 (any valid prefix of length n-2), giving the recurrence
    F(n) = F(n-1) + F(n-2), with F(0) = 1, F(1) = 2. *)
let solve n =
  let open BigNat in

  let rec iter fcur fprev n =
    if n = 1 then
      fcur
    else
      iter (add fcur fprev) fcur (n - 1)
  in iter two one n

let main () =
  let n = Scanf.scanf "%d" Fun.id in
  let s = solve n in
  Printf.printf "%s\n" (BigNat.to_string s)

let () = main ()
